...
% We replicated our findings using the Simons Genome Diversity Project (SGDP), which includes some African individuals from HGDP. We compared the local ancestry inferred for Mandenka, Bantu Kenya, Mbuti and Biaka individuals using the SGDP reference panel with that from the HGDP+1000GP reference panel and found a high correlation in local ancestry estimates, with similar proportions of Eurasian ancestry in both datasets (see Figure \ref{}). Additionally, we performed further analyses by adjusting the epoch boundaries and omitting the HMM, which resulted in highly correlated local ancestry and the same conclusions ($R^2 > zz$, see Figure \ref{fig:zz}).
